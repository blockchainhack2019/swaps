...
const { bitcoin, network, getPublicKey, fetchBalance, fetchUnspents, broadcastTx } = require('./btc')
...
const create = (data) => {
  ...
  const { secretHash, lockTime = _lockTime } = data

  const ownerPublicKey        = getPublicKey(Alice.accounts.btc)
  const participantPublicKey  = getPublicKey(Bob.accounts.btc)
  ...
  const contractAddress = bitcoin.address.fromOutputScript(publicKey, network)
...
const fund = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const unspents      = await fetchUnspents(Alice.accounts.btc.getAddress())
...
const _getWithdrawRawTransaction = async (data) => {
  ...
  const tx            = new bitcoin.TransactionBuilder(network)
  const unspents      = await fetchUnspents(contractAddress)
  const totalUnspent  = unspents.reduce((summ, { satoshis }) => summ + satoshis, 0)
  const feeValue      = 30000
